/*
 * guitar_tuner_completed_Chandima.c
 *
 * Created: 5/14/2016 11:55:23 AM
 *  Author: chandima
 */ 
...
/*
Standard E tuning
frequency period samples at 125kHz (0.2ms)
...
void serialbegin(int Speed){
	int16_t	tmp=Speed;//F_CPU/8/Speed-1;
	UBRRL=tmp;
	UBRRH=(tmp>>8);

	UCSRB=(1<<TXEN)|(1<<RXEN);//enable tx
	UCSRC=(1<<URSEL)|(1<<UCSZ1)|(1<<UCSZ0);//stop bit and size
}
void sendch(char a){
	while (( UCSRA & (1<<UDRE))  == 0){};
	UDR=a;
}
void usart(char var, FILE *stream) {
	if (var == '\n') sendch('\r');
	sendch(var);
}//hasith
static FILE mystdout = FDEV_SETUP_STREAM(usart, NULL, _FDEV_SETUP_WRITE);
// Sampling variables
...
void interrupt_Enable(){
	TIMSK |= 1<<OCIE0; // enable interupts on compare
}
